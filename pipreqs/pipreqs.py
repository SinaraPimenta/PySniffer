...
def get_all_imports(
    ...
    return list(packages - data),list(packages-(packages-data))
...
def generate_python_requirements(path, imports):
    with _open(path, "w") as out_file:
        for imp in imports:
            out_file.write(f"{imp}\n")
    #pensar na questão das deps desse script
...
def init(args):
    ...
    candidates,python_imports = get_all_imports(input_path,
                                 encoding=encoding,
